...
export async function createAssignment(req: Request, res: Response) {
  try {
    ...
    try {
      const types = JSON.parse(questionTypes);
    ...
    await assessmentQueue.add("generate", { assignmentId: assignment.id });
...
export async function getAssignmentResult(req: Request, res: Response) {
  try {
    ...
    if (!result) {
      return res.status(404).json({ error: "No generated results exist." });
    }

    const parsedSections = result.sections.map((sec: any) => ({
      ...sec,
...
export async function getAssignmentResultPdf(req: Request, res: Response) {
  try {
            ...
            <div class="section">
              <div class="section-header">
                ...
                <div class="section-desc">
                  ...
                  <div class="section-instruction">${sec.instruction || "Attempt all questions. Each question carries marks as indicated."}</div>
                </div>
              </div>
              <div>
                ${sec.questions.map((q: any, qIdx: number) => {
                  const opts: string[] | null = q.options ? JSON.parse(q.options) : null;
                  return `
                    <div class="question-item">
                      <div class="question-text-row">
                        <div style="flex-grow: 1;">
                          <span class="question-num">${qIdx + 1}.</span>
                          <span class="question-diff">[${q.difficulty.charAt(0).toUpperCase() + q.difficulty.slice(1).toLowerCase()}]</span>
                          <span>${q.text}</span>
                        </div>
          ...
          <div>
            ${result.sections.flatMap((s: any) => s.questions).map((q: any, idx: number) => `
              <div class="answer-item">
                <span class="question-num" style="shrink: 0;">${idx + 1}.</span>
    ...
    console.log("[PdfService] Compiling PDF");
    ...
    return res.status(500).json({ error: `PDF compiler error: ${error.message}` });
  }
}
...
export async function listAssignments(req: Request, res: Response) {
  try {
    ...
    return res.status(500).json({ error: "Failed to retrieve assignments list." });
  }
}
...
export async function deleteAssignment(req: Request, res: Response) {
  try {
    ...
    if (assignment.uploadedFile && fs.existsSync(assignment.uploadedFile)) {
      try {
        fs.unlinkSync(assignment.uploadedFile);
      } catch (fileErr: any) {
        console.error("[Controller] Failed to delete local assignment source file:", fileErr.message);
      }
    }
